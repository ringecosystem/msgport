...
library Utils {
    ...
    // input: 0x64616362
    // return: 1684104034
    function bytesToUint64(bytes memory input) internal pure returns (uint64 output) {
        require(input.length >= 8, "input must be at least 8 bytes");
        bytes memory inputWithLeftPadding = input;
        for (uint256 i = 0; i < 8 - input.length; i++) {
            inputWithLeftPadding = GNSPSBytesLib.concat(hex"00", inputWithLeftPadding);
        }
    ...
    function uint64ToBytes(uint64 input) internal pure returns (bytes memory output) {
        output = abi.encodePacked(input);
    }

    function bytesToUint16(bytes memory input) internal pure returns (uint16 output) {
        require(input.length >= 2, "input must be at least 2 bytes");
        bytes memory inputWithLeftPadding = input;
        for (uint256 i = 0; i < 2 - input.length; i++) {
            inputWithLeftPadding = GNSPSBytesLib.concat(hex"00", inputWithLeftPadding);
        }
        output = GNSPSBytesLib.toUint16(inputWithLeftPadding, 0);
    }

    function uint16ToBytes(uint16 input) internal pure returns (bytes memory output) {
        output = abi.encodePacked(input);
    }
    ...
    function hexStringToAddress(string memory addressString) internal pure returns (address) {
    ...
    function revertWithMessage(bytes memory returndata, string memory errorMessage) internal pure {
